...
class LoginAPI(Resource):
    def post(self):
        ...
        if not data:
            logger.warning("Отсутствует JSON тело",
                          extra=make_extra())
        ...
        if "username" not in data or "password" not in data:
            ...
            logger.warning("Отсутствуют обязательные поля",
                          extra=make_extra(data={"attempted_username": attempted_username}))
        ...
        if not user or not user.check_password(data["password"]):
            logger.warning("Неудачная попытка входа",
                           extra=make_extra(data={"attempted_username": data["username"]}))
        ...
        logger.info("Успешная аутентификация",
                    extra=make_extra(user_id=user.id, data={"username": user.username}))
        return {
...
class RefreshTokenAPI(Resource):
    ...
    def post(self):
        ...
        logger.info("Запрос обновления токена",
...
class LogoutAPI(Resource):
    ...
    def post(self):
        ...
        return {
            ...
            "note": "Access токен истечет через 15 минут, refresh через 7 дней"
        }, 200