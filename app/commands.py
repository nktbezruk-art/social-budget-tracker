...
def register_commands(app):
    ...
    @click.option('--dry-run', is_flag=True, help='Показать что удалит, но не удалять')
        ...
        if old_transactions:
            if dry_run:
                ...
                for transaction in old_transactions:
                    click.echo(f"{transaction.type} {str(transaction.amount)} {transaction.date.strftime('%d.%m.%Y %H:%M')}")
            else:
                count = query.delete()
                db.session.commit()
    ...
    def add_category(categories, details):
        ...
        for name in categories:
            ...
            if category:
                existed += 1
                if details:
                    click.echo(f"⏭️ Категория '{category.name}' уже существует")
            else:
                added += 1
        ...
        db.session.commit()
        if added > 0:
            click.secho(f"🎯 Добавлено {added} категорий", fg='green', bold=True)
        if existed > 0:
